...
export default function Sidebar() {
    ...
    return (
      <Link to={item.path}>
        <motion.div
          whileHover={{ x: 4 }}
          whileTap={{ scale: 0.98 }}
          className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${active ? "bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/25" : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"}`}
        >
  ...
  const SectionHeader = ({ title }) => (
    <div className="px-3 pb-2">
      {!collapsed && (
        <motion.h3 initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
          {title}
        </motion.h3>
      )}
    </div>
    ...
    >
        ...
        {!collapsed && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2">
            <div className="px-3 py-2 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center shadow-md">
              <span className="text-white font-bold text-sm whitespace-nowrap">MBOS LMS</span>
            </div>
          </motion.div>
        )}
        <button onClick={toggleSidebar} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors ml-auto">
          {collapsed ? <ChevronRightIcon className="h-5 w-5 text-gray-500" /> : <ChevronLeftIcon className="h-5 w-5 text-gray-500" />}
        </button>
      </div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="space-y-1">
              {mainMenu.map((item) => <MenuItem key={item.path} item={item} />)}
            </div>
          </div>
          ...
          {(user?.role === "MANAGER" || user?.role === "ADMIN") && (
            <div>
              ...
              <div className="space-y-1">
                {managerMenu.map((item) => <MenuItem key={item.path} item={item} />)}
              </div>
            </div>
          )}
          ...
          {isAdmin && (
            <div>
              ...
              <div className="space-y-1">
                {adminMenu.map((item) => <MenuItem key={item.path} item={item} />)}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-800">
        <div className="flex items-center gap-3">
          {profilePicture ? (
            <img src={profilePicture} alt="Profile" className="flex-shrink-0 w-10 h-10 rounded-full object-cover ring-2 ring-indigo-200 dark:ring-indigo-800" onError={() => setProfilePicture(null)} />
          ) : (
