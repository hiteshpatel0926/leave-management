...
export default function EmployeeDetails() {
  ...
  // ✅ FIXED: Ensure numeric sums (not string concatenation)
  const totalAllocated = employee.balances?.reduce((sum, b) => {
    const val = Number(b.entitled_days);
    return sum + (isNaN(val) ? 0 : val);
  }, 0) || 0;

  const totalUsed = employee.balances?.reduce((sum, b) => {
    const val = Number(b.used_days);
    return sum + (isNaN(val) ? 0 : val);
  }, 0) || 0;

  const totalBalance = employee.balances?.reduce((sum, b) => {
  ...
  return (
    ...
    >
      ...
      {/* Summary Stats Cards - FIXED with proper numbers */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md border border-gray-200 dark:border-gray-800 p-5 hover:shadow-lg transition">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Used Days</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white">
                {totalUsed}
              </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex gap-3 mt-1 text-xs">
                <span className="text-green-600">✓ {requestStats.approved}</span>
                <span className="text-yellow-600">⏳ {requestStats.pending}</span>
