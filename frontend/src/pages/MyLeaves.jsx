...
export default function MyLeaves() {
  ...
  const getStatusBadge = (status) => {
    const styles = {
      ...
      REJECTED:
        "bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400 border-rose-200 dark:border-rose-800/50",
      PENDING:
  ...
  // Direct session formatter – no helper needed
  const getSessionDisplay = (leave) => {
    if (parseFloat(leave.total_days) !== 0.5) return "Full Day";
  ...
  const data = filteredLeaves.map((leave) => [
    ...
    <span className="text-gray-600 dark:text-gray-400 text-sm">
      {getSessionDisplay(leave)}
    </span>,
    <span
      className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${getStatusBadge(leave.status)}`}
    >
      {leave.status}
    </span>,
    leave.status === "PENDING" ? (
      <button
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg text-rose-600 bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/20 dark:text-rose-400 dark:hover:bg-rose-900/40 transition-all"
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              Track the status of your past and present leave applications
            </p>
          </div>
        </div>
        <div className="relative">
          ...
          <select
            ...
            className="pl-11 pr-8 py-2.5 text-sm rounded-card border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-500 appearance-none shadow-sm cursor-pointer"
          >
      ...
      {filteredLeaves.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-card shadow-card border border-gray-100 dark:border-gray-700">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            You don't have any leave requests matching this filter.
          </p>
        </div>
      ) : (
