...
export default function MyProfile() {
  ...
  const handleUploadSuccess = (newUrl) => {
    setAvatar(newUrl + '?t=' + Date.now()); // cache bust
    loadProfile(); // refresh profile data
  };

  if (!profile) return <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500 mx-auto mt-10" />;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6 flex flex-col items-center gap-4 md:flex-row md:justify-start md:gap-6">
        ...
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-500 dark:text-gray-400">{profile.employee_code}</p>
          <button
            onClick={() => setShowCrop(true)}
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Employee Code</label>
            <p className="text-gray-900 dark:text-white font-medium">{profile.employee_code}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Full Name</label>
            <p className="text-gray-900 dark:text-white font-medium">{profile.first_name} {profile.last_name}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-gray-900 dark:text-white font-medium">{profile.dob ? new Date(profile.dob).toLocaleDateString() : "-"}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-gray-900 dark:text-white font-medium">{profile.designation}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Joining Date</label>
            <p className="text-gray-900 dark:text-white font-medium">{new Date(profile.joining_date).toLocaleDateString()}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${profile.status === "ACTIVE" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"}`}>
              {profile.status}
            </span>
          </div>
        </div>
