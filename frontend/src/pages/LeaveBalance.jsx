...
export default function LeaveBalance() {
  ...
  const years = [...new Set(allBalances.map((b) => b.year))].sort(
    (a, b) => b - a,
  );
  ...
  const activeBalances = filteredBalances.filter((b) =>
    activeBalanceCodes.includes(b.code),
  );

  const activeTotals = activeBalances.reduce(
    (acc, row) => {
  ...
  const allTotals = filteredBalances.reduce(
    (acc, row) => {
      acc.entitled += parseFloat(row.entitled_days) || 0;
      acc.used += parseFloat(row.used_days) || 0;
      acc.balance += parseFloat(row.balance_days) || 0;
  ...
  const additionalEntitlements = filteredBalances.filter((b) =>
    additionalLeaveCodes.includes(b.code),
  );
  ...
  const data = filteredBalances.map((row) => [
    ...
    <span className="inline-flex font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1 rounded-lg">
      {row.balance_days}
    </span>,
  ]);
  ...
  if (filteredBalances.length > 0) {
    data.push([
      ...
      <span></span>,
      <span className="font-bold text-gray-900 dark:text-white">
        {allTotals.entitled.toFixed(2)}
      </span>,
      <span className="font-bold text-gray-900 dark:text-white">
        {allTotals.used.toFixed(2)}
      </span>,
        ...
        {years.length > 0 && (
          <div className="relative">
            ...
            <select
              ...
              className="pl-10 pr-8 py-2.5 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 appearance-none cursor-pointer shadow-sm"
            >
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                  Total Allocated (PL + CO)
                </p>
                <p className="text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {activeTotals.entitled.toFixed(1)}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  days available for this year
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-gray-100 dark:border-gray-700 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {activeTotals.balance.toFixed(1)}
                </p>
